...
class dot(sympy.core.operations.LatticeOp):
    """my dot product"""
...
def to_latex(expr):
    ...
    if expr_s[0] == '$':
        ...
        return Markup('[mathjax]{expression}[/mathjax]<br>').format(expression=expr_s[1:-1])	 # for sympy v6
    # return HTML('[mathjax]{expression}[/mathjax]<br>').format(expression=expr_s)		# for sympy v7
    return Markup('[mathjax]{expression}[/mathjax]<br>').format(expression=expr_s)		# for sympy v7
...
class formula():
    ...
    def get_content_mathml(self):  # lint-amnesty, pylint: disable=missing-function-docstring
        ...
        pmathml = etree.tostring(xml, pretty_print=True).decode('utf-8')
    ...
    def make_sympy(self, xml=None):  # lint-amnesty, pylint: disable=too-many-statements
        ...
        # atoms are below
            ...
            return my_sympify("".join([x.text for x in xml]))
